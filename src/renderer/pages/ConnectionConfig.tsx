...
export const ConnectionConfig: React.FC = () => {
    ...
    const [tunnelProcessState, setTunnelProcessState] = useState<TunnelProcessState>('idle')
    ...
    useEffect(() => {
        fetchConfig()
        fetchTunnelConfigs()
        window.api.tunnelExecution.getState().then(({state}) => setTunnelProcessState(state))
        return window.api.tunnelExecution.onStateChanged(({state}) => setTunnelProcessState(state))
    }, [])
    ...
    const handleTunnelSave = async () => {
        ...
        try {
            if (tunnelDialogMode === 'add') {
                ...
                const updateConfig: UpdateTunnelConfig = {
                    id: selectedTunnelId,
                    name: tunnelName.trim(),
                    command: tunnelCommand.trim()
                }
        ...
        <Box p={3} sx={{maxWidth: 800, margin: '0 auto'}}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
                ...
                <Box display="flex" gap={1}>
                    ...
                    />
                    <Chip
                        label={tunnelProcessState === 'running' ? t('tunnel.chip_running') : tunnelProcessState === 'stopped' ? t('tunnel.chip_stopped') : tunnelProcessState === 'error' ? t('tunnel.chip_error') : t('tunnel.chip_not_configured')}
                        color={tunnelProcessState === 'running' ? 'success' : tunnelProcessState === 'error' ? 'error' : tunnelProcessState === 'stopped' ? 'warning' : 'default'}
                    />
                </Box>
            </Box>
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid size={{xs: 12, sm: 6}}>
                        <TextField
                            ...
                            slotProps={{input: {inputProps: {min: 1024, max: 65535}}}}
                        />
                    </Grid>
                    {editAddress === '0.0.0.0' && (
                        <Grid size={12}>
            ...
            <Dialog open={regenerateDialogOpen} onClose={() => setRegenerateDialogOpen(false)}>
                ...
                <DialogActions>
                    <Button
                        onClick={() => setRegenerateDialogOpen(false)}>{t('connection.regenerate_confirm_no')}</Button>
