...
**Created**: 2026-04-01  
**Status**: Draft  
**Input**: User description: "Use Chain of Responsibility pattern as the core architecture. The engine exposes a single entry point that runs a list of SecurityCheck objects in order and stops on first block. Define these TypeScript interfaces in src/main/security/types.ts: SecurityCheck with readonly name (string) and validate(context: CommandContext): SecurityVerdict; CommandContext with rawInput (string), normalizedInput (string), segments (string[]); SecurityVerdict with allowed (boolean), reason (optional string), matchedRule (optional string), checkName (optional string). The SecurityEngine class in src/main/security/engine.ts accepts an array of SecurityCheck in its constructor and exposes a single validate(rawInput: string): SecurityVerdict method that runs normalization first, then chain splitting into segments, then iterates checks in order. BlocklistCheck lives in src/main/security/checks/blocklist.check.ts and is the only check implemented in this feature. The blocklist is a readonly frozen object exported from src/main/security/blocklist.ts, organized by category where each entry has pattern as RegExp, category as string, and severity as either high or critical. Normalization runs before chain splitting and lives in src/main/security/utils/normalize.ts — it collapses whitespace, strips quote escaping, detects Unicode homoglyphs and zero-width spaces, and strips inline shell comments. Chain splitting lives in src/main/security/utils/split-commands.ts — it splits on semicolon, &&, ||, pipe, backticks, $(...), and heredocs and returns each segment as an individual string. Regex library: Node.js built-in RegExp only, no external parser library. Unicode normalization: String.prototype.normalize('NFC'), no external library. The Security Engine is NOT reachable via IPC — it is an internal Main process module only, no contextBridge exposure, no IPC handler. There is no contracts/ipc.md needed for this feature. data-model.md covers TypeScript types only, no DB entities: BlocklistEntry (pattern RegExp, category string, severity high or critical), CommandContext, SecurityVerdict. All files are pure TypeScript with no Electron, no DB, no IPC dependencies. Path validation is explicitly out of scope and will be a future PathCheck chain link. Symlink resolution is out of scope. Script file content inspection (bash script.sh) is out of scope and noted for a future feature."
